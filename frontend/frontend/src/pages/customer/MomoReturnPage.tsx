...
export default function MomoReturnPage() {
  ...
  const isMembership = queryParams.orderId?.startsWith('MEMBERSHIP_') ?? false;
  ...
  useEffect(() => {
    ...
    const processPayment = async () => {
      try {
        if (!queryParams.orderId || !queryParams.requestId || !queryParams.signature) {
          if (isMounted) setStatus('failed');
          return;
        }

        const result = await paymentApi.confirmMomoReturn(queryParams);
        if (!isMounted) return;
        ...
        if (!isMounted) return;
        setStatus('failed');
        toast.error('Có lỗi xảy ra khi xác nhận thanh toán MoMo.');
      }
    };
      ...
      <div className="w-full max-w-md rounded-2xl border border-white/70 bg-white/85 p-8 text-center shadow-[0_24px_70px_rgba(15,23,42,0.14)] backdrop-blur-xl">
        {status === 'loading' && (
          <div className="flex flex-col items-center">
            <Loader2 className="mb-4 h-16 w-16 animate-spin text-pink-600" />
